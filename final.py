...
SAVE_WEIGHTS = False
LOAD_WEIGHTS = False
INPUT_TITLE = 'data_files/amj_data3'
OUTPUT_TITLE = 'results/amj_data'
TRAIN_MODEL_1 = False
TRAIN_MODEL_2 = False
TRAIN_MODEL_3 = True
...
if (TRAIN_MODEL_1):
	train1, test1, validation1 = data.getFinalData(INPUT_TITLE + '.xlsx')
...
if (TRAIN_MODEL_2):
	train2, test2, validation2 = data.getFinalData(INPUT_TITLE + '.xlsx')
...
if (TRAIN_MODEL_3):
	train3, test3, validation3 = data.getFinalData(INPUT_TITLE + '.xlsx')
...
if (TRAIN_MODEL_1):
	...
	if (SAVE_OUTPUT):
		...
		data.saveData(result, OUTPUT_TITLE + '1.xlsx')
...
if (TRAIN_MODEL_2):
	...
	if (SAVE_OUTPUT):
		...
		data.saveData(result, OUTPUT_TITLE + '2.xlsx')
...
if (TRAIN_MODEL_3):
	...
	if (SAVE_OUTPUT):
		...
		data.saveData(result, OUTPUT_TITLE + '3.xlsx')
...
for layer in model3.layers:
        ...
        for weight in layer.get_weights():
            ...
            tmp = np.array(weight)
            data.saveData(tmp, 'omer_weights' + str(counter) + '.xlsx')
            #print('shape:weight:', tmp.shape)
            counter = counter + 1